...
The following is a list of implementations specifics which describe the particular way in which the Minlog system implements program extraction:

\begin{itemize}
\item The disjunction $\vee$ operation is defined as an inductive defined predicate \texttt{ori} with closure axioms instead of the rules 

\item $\exists$ is treated by axioms instead of rules.

\item The nc-quantifiers are treated in the same way as their non-nc counter parts in terms of how natural deduction is implemented. 

\item Program constants are given computational behaviour using rewriting rules.

\item Inductively defined predicates are given computational behaviour by their closure axioms.
...
The proof proceeds by performing induction on the proof term $M$ we omit further details.

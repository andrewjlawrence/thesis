...
\begin{lstlisting}
(GRecGuard ρ1 ρ2 ρ3 τ) μ z1 z2 z3 G t = f z1 z2 z3 t where
f : ρ1 ⇒ ρ2 ⇒ ρ3 ⇒ Boole ⇒ τ
f x1 x2 x3 True = G x x2 x3 ([y1, y2, y3].f y1 y2 y3(μ y1 y2 y3 < μ x1 x2 x3))
...
Similarly, the extracted program for non-measure induction is a recursion operation \texttt{Rec}.
 ...
 it is possible to write the operation Rec list ρ ⇒ τ using a function
...
\section{Other Theorem Provers and Their Foundations}
Unlike Minlog the following 3 interactive theorem provers all have type theoretic foundations
\subsection*{Isabelle} 
The theorem prover Isabelle \cite{TN99,LP94}
\subsection*{Coq} 
The theorem prover Coq \cite{YB04}

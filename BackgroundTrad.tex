\thesischapter{Background: Traditional Railway Control Systems}{Traditional Railway Control Systems}
...
The most important type of signal found in the modern railway is the \textbf{fixed signal} which is static, positioned by the side of the track and visually present information to the train driver. The first fixed signals were wooden boards shaped in such a way to provide specific information attached to poles which rotate around a vertical axis. Typically these boards would form a signal instructing the train driver to stop, however if the board was rotated side-on to the driver then it becomes inactive and the driver would proceed if it is safe to do so.

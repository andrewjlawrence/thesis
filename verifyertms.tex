...
\subsection*{Checking correctness of the specification}

We have applied model checking to check several fundemental correctness properties of the pentagon examples behaviour. We begin by showing that it is not possible for a train to exceed its max speed. We formulate this as a model checking problem over the pentagon example. 
...
MA1 (Pentagon)& 30 &77.724s  \\
MA2 (Pentagon)& 30 &79.192s \\ 
MA3 (Pentagon)&  30 &79.380s \\
P1 (Junction)& untimed &7.488s \\
P2 (Junction)& untimed & 6.636s
\end{tabular}
\end{center}
The removal of the non-determinism and finite state space of the junction example has made a large improvement to the model checking of the safety properties.

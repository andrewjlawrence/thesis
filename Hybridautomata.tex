...
We define the breaking distance for a train in terms of its speed S and acceleration A is: 
...
We use the following abbreviation to capture the two seperate events and flow conditions for train movement in the automaton diagram:
\begin{align*}
...
\begin{description}
\item[Variables] The state of the interlocking automaton consists of five boolean variables  $\underbrace{l_0, \ldots , l_4}_\text{Occupied/Free}$ and a variable $ReqID$ ranging over $\{0 , \ldots , 4 \}$ storing the last requested track segement.

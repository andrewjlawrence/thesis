...
\begin{itemize}
	...
	\item $event(Full \, Speed \to Full \, Speed) := event(Braking \to Braking)$ \\
                  $:= event(Accelerating \to Accelerating)  := l_{D \, div \, 50}.l_{(D \, div \, 50) +1}$
\item $event(Full \, Speed \to Full \, Speed (2)) := event(Braking \to Braking (2))$  \\
        $:= event(Accelerating \to Accelerating(2)) := Train.TrainID.Pos.D$

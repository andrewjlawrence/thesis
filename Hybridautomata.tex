...
The message sequence diagram Figure \ref{fig:SeqChart2} shows the communication between the RBC, interlocking and Train 1 from the pentagon example. Train 1 initially sends position updates to the RBC until it reaches its braking point, at which time it will request a movement authority the RBC. The RBC will then request the next track segment for the train, which in this case is immediately granted. The RBC then sends the train its new movement authority.
\begin{figure} [h!]

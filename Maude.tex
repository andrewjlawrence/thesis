...
\todo{Do we need a  definition of an equational theory morphism?}
...
\begin{figure}
\includegraphics{t1graph.png}
\capture{A graph comparing the distance and speed of train1}
\end{figure}

\begin{figure}
\includegraphics{t2graph.png}
\caption{A graph comparing the distance and speed of train2}
\end{figure}

\begin{figure}
\label{t1t2graph}
\includegraphics{t1t2graph.png}
...
In the following section we will demonstrate an approach to apply the \rtmaude LTL model checker to verify the \rtmaude specification described previously.
Firstly we shall define the property we want to check in the Real Time Maude system. To do this we have to define a satisfaction relation which describes what it means for the property to hold in a state of the system we are checking. Then we describe the results of applying the \rtmaude LTL model checker to verify the system. 

\subsection*{Defining a Satisfaction Relation}
We shall now describe how to define the behaviour of the satisfaction relation for a given system and property as a \rtmaude specification. The satisfaction relation itself is predefined in a specification. Maude defines the satisfaction relation $\models$ in the following module:
\begin{verbatim}
...
The \rtmaude LTL model checker is then called using the following command:
